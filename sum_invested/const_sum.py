...
def ema_two_overlay(ticker,amt,
                         per1 = '20y',
                         int1 = '1wk',
                         short1 = 9,
                         long1 = 26,
                         ...
                         short2 = 9,
                         long2 = 26,
                        path_for_file = r"C:\Users\Admin\Desktop\algofin\sum_invested"):
    ...
    for i in range(0,len(df3_mini)):
        ...
        if (df3_mini.iloc[i,4]+df3_mini.iloc[i,5]) == 3:
            ...
            df3_mini.iloc[i,8] = int(amt/df3_mini.iloc[i,0])
    ...
    df3_mini['Gains'] = 0
    df3_mini["%gain in trade"] = 0

    df3_mini['Holding period'] = 0
    ...
    for i in range(0,len(df3_mini)):
        ...
        if df3_mini.iloc[i,6] == 0 and df3_mini.iloc[i-1,6] == 1 and i!=0 :   #long then now neutral
            df3_mini.iloc[i, 9] = (df3_mini.iloc[i-1,8])*(df3_mini.iloc[i,0] - df3_mini.iloc[i-1,0])   #n * difference
            df3_mini.iloc[i,10] = (df3_mini.iloc[i, 9]/amt)
            df3_mini.iloc[i,11] = (df3_mini.index[i] - df3_mini.index[i-1]).days
        elif df3_mini.iloc[i,6] == 0 and df3_mini.iloc[i-1,6] == -1 and i!=0 :  # short then now neutral
            df3_mini.iloc[i, 9] = (df3_mini.iloc[i-1,8])*(df3_mini.iloc[i, 0] - df3_mini.iloc[i-1, 0])  #-n*-diff
            df3_mini.iloc[i, 10] = (df3_mini.iloc[i, 9] / amt)
            df3_mini.iloc[i, 11] = (df3_mini.index[i] - df3_mini.index[i-1]).days
...
print(ema_two_overlay(ticker='RELIANCE.NS',amt=100000, per1 = '20y',
                         int1 = '1wk',
                         short1 = 9,
                         long1 = 26,
